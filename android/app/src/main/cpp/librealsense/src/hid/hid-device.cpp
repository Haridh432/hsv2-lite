    ...
    namespace platform
    {
        ...
        void rs_hid_device::start_capture(hid_callback callback)
        {
            _action_dispatcher.invoke_and_wait([this, callback](dispatcher::cancellable_timer c)
            {
                ...
                _request_callback = std::make_shared<usb_request_callback>([&](platform::rs_usb_request r)
                    {
                        _action_dispatcher.invoke([this, r](dispatcher::cancellable_timer c)
                        {
                            if(!_running)
                                return;
                            static std::atomic<unsigned long long> imu_hid_packet_count(0);

                            const auto hid_count = ++imu_hid_packet_count;

                            if (hid_count % 200 == 0)
                            {
                                LOG_INFO(
                                    "IMU HID callback packets="
                                    << hid_count
                                    << " actual_length="
                                    << r->get_actual_length()
                                    << " expected="
                                    << sizeof(REALSENSE_HID_REPORT));
                            }
                ...
                LOG_INFO(
                    "IMU DEBUG: creating HID requests count="
                    << USB_REQUEST_COUNT);

                auto imu_endpoint = get_hid_endpoint();

                LOG_INFO(
                    "IMU DEBUG: request endpoint="
                    << (int)imu_endpoint->get_address());

                for(auto&& r : _requests)
                {
                    r = _messenger->create_request(imu_endpoint);

                    LOG_INFO(
                        "IMU DEBUG: create_request result="
                        << (r ? "NON_NULL" : "NULL"));
...
#ifndef __APPLE__
                LOG_INFO(
                    "IMU DEBUG: submitting HID requests count="
                    << _requests.size());
                ...
                for(auto&& r : _requests)
                {
                    ...
                    LOG_INFO(
                        "IMU DEBUG: submit_request status="
                        << (int)submit_status);
                }
#endif
        ...
        {
            LOG_INFO(
                "IMU SET_FEATURE_REPORT called power=" << (int)power
                << " report_id=" << report_id
                << " fps=" << fps);
            ...
            if (!dev)
            {
                LOG_INFO("IMU SET_FEATURE_REPORT: no device (RS2_USB_STATUS_NO_DEVICE)");
            ...
            LOG_INFO("IMU SET_FEATURE_REPORT: first GET res=" << (int)res
                << " transferred=" << transferred
                << " power(before)=" << (int)featureReport.power);
            ...
            LOG_INFO("IMU SET_FEATURE_REPORT: SET res=" << (int)res
                << " transferred=" << transferred
                << " power(set)=" << (int)featureReport.power
                << " report(set)=" << (int)featureReport.report);
        ...
        rs_usb_endpoint rs_hid_device::get_hid_endpoint()
        {
            ...
            LOG_INFO(
                "IMU HID endpoint selected: address="
                << (int)ep->get_address()
                << " direction="
                << (int)ep->get_direction()
                << " type="
                << (int)ep->get_type());
