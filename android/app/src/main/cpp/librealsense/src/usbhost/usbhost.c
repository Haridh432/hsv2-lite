                                    ...
                                    const struct usb_endpoint_descriptor *ep_desc)
{
    ...
    {
        __android_log_print(
            ANDROID_LOG_INFO, IMU_LOG_TAG,
            "IMU_NEW endpoint=%d attr=%d maxPacket=%d interval=%d type=%d",
            ep_desc->bEndpointAddress,
            ep_desc->bmAttributes,
            __le16_to_cpu(ep_desc->wMaxPacketSize),
            ep_desc->bInterval,
            urb->type);
    }

    if (ep_desc->bEndpointAddress == 134)
    {
        D("[IMU REQUEST NEW] endpoint=%d attributes=%d maxPacket=%d interval=%d type=%d\\n",
          ep_desc->bEndpointAddress,
          ep_desc->bmAttributes,
          __le16_to_cpu(ep_desc->wMaxPacketSize),
          ep_desc->bInterval,
...
int usb_request_queue(struct usb_request *req)
{
    ...
    do {
        static unsigned long long submit_count = 0;
        submit_count++;

        if (req->endpoint == 134 || submit_count <= 10)
        {
            D("[USB SUBMIT] count=%llu endpoint=%d buffer_length=%d\\n",
              submit_count,
              req->endpoint,
              req->buffer_length);
        }
        ...
        if (req->endpoint == 134)
        {
            __android_log_print(
                ANDROID_LOG_INFO, IMU_LOG_TAG,
                "IMU_SUBMIT endpoint=134 res=%d errno=%d buffer_length=%d",
                res, errno, req->buffer_length);
        }

        if (req->endpoint == 134)
        {
            D("[USB SUBMIT RESULT] endpoint=%d res=%d errno=%d\\n",
              req->endpoint,
              res,
              errno);
        }
...
struct usb_request *usb_request_wait(struct usb_device *dev, int timeoutMillis)
{
    ...
    if (timeoutMillis > 0) {
        ...
        if (p.fd >= 0) {
            __android_log_print(
                ANDROID_LOG_INFO, IMU_LOG_TAG,
                "IMU_POLL res=%d revents=0x%x errno=%d timeout=%d",
                res, p.revents, errno, timeoutMillis);
        }
    ...
    if (res < 0) {
        __android_log_print(
            ANDROID_LOG_INFO, IMU_LOG_TAG,
            "IMU_REAP_FAIL res=%d errno=%d timeoutMillis=%d",
            res, errno, timeoutMillis);
        D("[ reap urb - error %d]\n", errno);
        return NULL;
    } else {
        __android_log_print(
            ANDROID_LOG_INFO, IMU_LOG_TAG,
            "IMU_REAP_OK status=%d actual_length=%d",
