        ...
        usb_status usb_device_usbhost::submit_request(const rs_usb_request& request)
{
    ...
    const auto endpoint = request->get_endpoint()->get_address();

    if (endpoint == 134)
    {
        LOG_INFO("IMU USB submit_request: endpoint=134 BEFORE usb_request_queue");
    }
    ...
    if (endpoint == 134)
    {
        LOG_INFO("IMU USB submit_request: endpoint=134 AFTER usb_request_queue sts="
                 << sts << " errno=" << errno);
    }
    ...
    if(sts < 0)
    {
        ...
        LOG_WARNING("usb_request_queue returned error, endpoint: "
    ...
    return RS2_USB_STATUS_SUCCESS;
        ...
        void usb_device_usbhost::invoke()
        {
            ...
            {
                static std::atomic<unsigned long long> usb_wait_count(0);

                const auto wait_count = ++usb_wait_count;

                if (wait_count <= 5 || wait_count % 200 == 0)
                {
                    LOG_INFO(
                        "USB request_wait entered count="
                        << wait_count);
                }
                ...
                {
                    LOG_INFO(
                        "USB request_wait returned count="
                        << wait_count
                        << " request="
                        << (r ? "NON_NULL" : "NULL"));
                }
                ...
                if(r)
                {
                    ...
                    if(urb && urb->get_shared())
                    {
                        auto response = urb->get_shared();
                        if(response->get_endpoint()->get_address() == 134)
                        {
                            LOG_INFO(
                                "IMU USB completion received: endpoint=134 actual_length="
                                << response->get_actual_length());
                        }
                    }
                    ...
                    if(urb)
                    {
                        ...
                        if(response)
                        {
                            static std::atomic<unsigned long long> completed_usb_count(0);
                            const auto completed_count = ++completed_usb_count;

                            if (completed_count <= 10 || completed_count % 200 == 0)
                            {
                                LOG_INFO(
                                    "USB request completed count="
                                    << completed_count
                                    << " endpoint="
                                    << (int)response->get_endpoint()->get_address()
                                    << " actual_length="
                                    << response->get_actual_length());
                            }
