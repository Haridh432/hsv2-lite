                ...
                do {
                    ...
                    else if (val > 0)
                    {
                        ...
                        else if (FD_ISSET(_fd, &fds))
                        {
                            ...
                            static std::atomic<unsigned long long> hid_packet_count(0);
                            const auto hid_count =
                                    ++hid_packet_count;

                            if (hid_count % 200 == 0) {
                                LOG_INFO(
                                        "HID RAW packets=%llu read_size=%zu",
                                        hid_count,
                                        read_size);
                            }
                        }
...
//                            std::cout << std::dec << std::endl;
        ...
        void v4l_hid_device::foreach_hid_device(std::function<void(const hid_device_info&)> action)
        {
            ...
            for (auto& elem : common_sensors)
            {
                ...
                LOG_INFO(
                        "HID ENUM IIO: id=%s path=%s unique=%s",
                        hid_dev_info.id.c_str(),
                        hid_dev_info.device_path.c_str(),
                        hid_dev_info.unique_id.c_str());
            ...
            for (auto& elem : custom_sensors)
            {
                ...
                LOG_INFO(
                        "HID ENUM CUSTOM: id=%s path=%s unique=%s",
                        hid_dev_info.id.c_str(),
                        hid_dev_info.device_path.c_str(),
                        hid_dev_info.unique_id.c_str());
