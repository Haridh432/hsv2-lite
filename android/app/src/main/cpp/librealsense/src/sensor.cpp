                       ...
                       unsigned long long frame_number )
{
    ...
    {
        LOG_INFO("IMU HID_SENSOR_OPEN_CALLED tid=" << std::this_thread::get_id());
        std::lock_guard<std::mutex> lock(_configure_lock);
        if (_is_streaming)
    ...
    {
        LOG_INFO("IMU HID_SENSOR_CLOSE_CALLED tid=" << std::this_thread::get_id()
    ...
    {
        LOG_INFO("IMU HID_SENSOR_START_CALLED tid=" << std::this_thread::get_id());
        std::lock_guard<std::mutex> lock(_configure_lock);
        if (_is_streaming)
        ...
        _is_streaming = true;
        LOG_INFO("IMU HID_SENSOR_START_COMPLETED tid=" << std::this_thread::get_id());
    }

    void hid_sensor::stop()
    {
        LOG_INFO("IMU HID_SENSOR_STOP_CALLED tid=" << std::this_thread::get_id());
        std::lock_guard<std::mutex> lock(_configure_lock);
        if (!_is_streaming)
